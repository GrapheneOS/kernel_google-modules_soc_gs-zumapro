...
/* trace/hooks/fault.h */
static void dbg_snapshot_do_dpm_die_kernel_fault(void *data, const char *msg, unsigned long addr,
						 unsigned int esr, struct pt_regs *regs)
{
	dbg_snapshot_do_dpm(regs, esr);
}

static void dbg_snapshot_do_dpm_do_sea(void *data, unsigned long addr, unsigned int esr,
				       struct pt_regs *regs)
{
	dbg_snapshot_do_dpm(regs, esr);
}

static void dbg_snapshot_do_dpm_do_mem_abort(void *data, unsigned long addr, unsigned int esr,
					     struct pt_regs *regs)
{
	dbg_snapshot_do_dpm(regs, esr);
}

static void dbg_snapshot_do_dpm_do_sp_pc_abort(void *data, unsigned long addr, unsigned int esr,
					       struct pt_regs *regs)
{
	dbg_snapshot_do_dpm(regs, esr);
}
...
static void dbg_snapshot_do_dpm_do_ptrauth_fault(void *data, struct pt_regs *regs, unsigned int esr)
{
	dbg_snapshot_do_dpm(regs, esr);
}

static void dbg_snapshot_do_dpm_panic_unhandled(void *data, struct pt_regs *regs,
						const char *vector, unsigned int esr)
{
	dbg_snapshot_do_dpm(regs, esr);
}

static void dbg_snapshot_do_dpm_arm64_serror_panic(void *data, struct pt_regs *regs,
						   unsigned int esr)
{
	dbg_snapshot_do_dpm(regs, esr);
}
...
static void register_dbg_snapshot_do_dpm_vendor_hooks(void)
{
	...
	if (register_trace_android_rvh_do_ptrauth_fault(dbg_snapshot_do_dpm_do_ptrauth_fault,
	    NULL)) {
		pr_err("dpm: register do_ptrauth_fault hook failed\n");
		return;
	}

	if (register_trace_android_rvh_panic_unhandled(dbg_snapshot_do_dpm_panic_unhandled, NULL)) {
		pr_err("dpm: register panic_unhandled hook failed\n");
		return;
	}

	if (register_trace_android_rvh_arm64_serror_panic(dbg_snapshot_do_dpm_arm64_serror_panic,
	    NULL)) {
