...
#define S2MPG1415_WRITE(pmic, bcl_dev, ret, args...)                             \
	do {                                                                     \
		switch (pmic) {                                                  \
		case SUB:                                                        \
			ret = s2mpg15_write_reg((bcl_dev)->sub_pmic_i2c, args);  \
			break;						         \
		case MAIN:                                                       \
			ret = s2mpg14_write_reg((bcl_dev)->main_pmic_i2c, args); \
			break;						         \
		}                                                                \
	} while (0)

#define S2MPG1415_READ(pmic, bcl_dev, ret, args...)                              \
	do {                                                                     \
		switch (pmic) {                                                  \
		case SUB:                                                        \
			ret = s2mpg15_read_reg((bcl_dev)->sub_pmic_i2c, args);   \
			break;						         \
		case MAIN:                                                       \
			ret = s2mpg14_read_reg((bcl_dev)->main_pmic_i2c, args);  \
			break;						         \
		}                                                                \
	} while (0)
...
static ssize_t smpl_lvl_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	S2MPG1415_READ(MAIN, bcl_dev, ret, SMPL_WARN_CTRL, &value);
	if (ret)
		return -EINVAL;
			      ...
			      struct device_attribute *attr, const char *buf, size_t size)
{
	...
	S2MPG1415_WRITE(MAIN, bcl_dev, ret, SMPL_WARN_CTRL, value);
		       ...
		       u16 llimit, u16 ulimit, u16 step, u8 id)
{
	...
	S2MPG1415_WRITE(pmic, bcl_dev, ret, addr, value);
	if (!ret) {
...
static int google_set_sub_pmic(struct bcl_device *bcl_dev)
{
	...
	S2MPG1415_READ(SUB, bcl_dev, ret, SUB_CHIPID, &val);
	if (ret) {
...
static int google_set_intf_pmic(struct bcl_device *bcl_dev)
{
	...
	S2MPG1415_READ(MAIN, bcl_dev, ret, MAIN_CHIPID, &val);
	if (ret) {
...
static int google_set_main_pmic(struct bcl_device *bcl_dev)
{
	...
	/* see b/215371539 */
	S2MPG1415_READ(MAIN, bcl_dev, ret, S2MPG14_PM_OFFSRC1, &val);
	dev_info(bcl_dev->device, "MAIN OFFSRC1 : %#x\n", val);
	bcl_dev->offsrc = val;
	S2MPG1415_READ(MAIN, bcl_dev, ret, S2MPG14_PM_OFFSRC2, &val);
	dev_info(bcl_dev->device, "MAIN OFFSRC2 : %#x\n", val);
	S2MPG1415_READ(MAIN, bcl_dev, ret, S2MPG14_PM_PWRONSRC, &val);
	dev_info(bcl_dev->device, "MAIN PWRONSRC: %#x\n", val);
	bcl_dev->pwronsrc = val;
	S2MPG1415_WRITE(MAIN, bcl_dev, ret, S2MPG14_PM_OFFSRC1, 0);
	S2MPG1415_WRITE(MAIN, bcl_dev, ret, S2MPG14_PM_OFFSRC2, 0);
	S2MPG1415_WRITE(MAIN, bcl_dev, ret, S2MPG14_PM_PWRONSRC, 0);
