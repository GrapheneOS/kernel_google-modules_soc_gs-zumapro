...
extern void bbd_parse_asic_data(unsigned char *pucData, unsigned short usLen,
		void (*to_gpsd)(unsigned char *packet, unsigned short len,
			void *priv), void *priv);
...
/* 0 - Half Duplex, 1 - Full Duplex (not supported yet) */
int ssi_mode = 1;
/*  1 = 1B, 2 = 2B */
int ssi_len = 2;
/*
 * 0 - Flow Control Disabled, >0 -
...
static unsigned long g_rx_buffer_avail_bytes = HSI_PZC_MAX_RX_BUFFER;
...
void bcm_ssi_print_trans_stat(struct bcm_spi_priv *priv)
{
	...
	pr_info("DBG SPI @ TX: <255B = %d, <1K = %d, <2K = %d, <4K = %d, <8K = %d, <16K = %d, <32K = %d, <64K = %d, total = %ld, min = %ld, max = %ld",
	...
	pr_info("DBG SPI @ RX: <255B = %d, <1K = %d, <2K = %d, <4K = %d, <8K = %d, <16K = %d, <32K = %d, <64K = %d, total = %ld, min = %ld, max = %ld",
			trans->len_255, trans->len_1K, trans->len_2K,
			trans->len_4K, trans->len_8K, trans->len_16K,
			trans->len_32K, trans->len_64K, trans->len_total,
			trans->len_min, trans->len_max);

	pr_info("DBG SPI @ PZC: retries = %d, delays = %d",
		...
		unsigned char stat_byte, bool bprint)
{
	...
	if (stat_byte & HSI_F_MOSI_CTRL_PE_MASK) {
		pr_debug("DBG SPI @ PZC: rx stat 0x%02x%s avail %lu",
			stat_byte,
			stat_byte & HSI_F_MOSI_CTRL_PE_MASK ?  "(PE)" : "   ",
			g_rx_buffer_avail_bytes);
	}
...
static int bcm_spi_open(struct inode *inode, struct file *filp)
{
	...
	duplex_mask =
		ssi_mode != 0 ? SSI_MODE_FULL_DUPLEX : SSI_MODE_HALF_DUPLEX;
...
#ifdef DEBUG_1HZ_STAT
	pr_info("tx ctrl %02X: total %d = len %d + fc %d + cmd 1",
...
#ifdef DEBUG_1HZ_STAT
	pr_info("rx ctrl %02X: total %d = len %d + fc %d + stat 1\n",
			strm->ctrl_byte, strm->ctrl_len,
			strm->pckt_len, strm->fc_len);

	pr_info("SPI @ %d: %s Duplex Strm mode, %dB Len, %s FC, Frame Len %u : tx ctrl %02X, rx ctrl %02X\n",
			g_bcm_bitrate,
...
#define MAX_RESP_CHECK_COUNT 100 /* 100 msec */
	...
	if (count > 100)
		pr_info(" hello consumed %lu = clock_get_ms() - start_time; msec",
				delta);
		...
		void *rx_buf, int len, int bits_per_word)
{
	...
	if (ret)
		pr_err(" spi_sync error for cmd:0x%x, return=%d\n",
...
static int bcm_ssi_tx(struct bcm_spi_priv *priv, int length)
{
	...
	if (strm->pckt_len != 0) {
		...
		if (m_write > frame_data_size) {
			...
			pr_err("@ TX m_write %d is h/w overflowed of frame %d...Fail\n",
	...
	if (strm->ctrl_byte & SSI_MODE_FULL_DUPLEX) {
		...
		if (n_read > frame_data_size) {
			pr_err("@ FD n_read %d is h/w overflowed of frame %d...Fail\n",
...
int bcm_ssi_rx(struct bcm_spi_priv *priv, size_t *length)
{
	...
	if (payload_len == 0) {
		 ...
		 */
		pr_err("@ RX length is still read to 0. Set %d\n", payload_len);
		return -1;
	}
...
void bcm_check_overrun(struct bcm_spi_priv *priv, size_t avail)
{
	...
	if (priv->skip_count)
		pr_err("%ld messages are skipped!\n", priv->skip_count);

	pr_err("input overrun error by %zu bytes.\n", avail);
	priv->skip_count = 0;
...
void bcm477x_debug_info(struct bcm_spi_priv *priv)
{
	...
	irq_count = kstat_irqs_cpu(priv->spi->irq, 0);

	pr_info("pin_ttyBCM:%d, pin_MCU_REQ:%d, pin_MCU_RESP:%d\n",
		pin_ttyBCM, pin_MCU_REQ, pin_MCU_RESP);
	pr_info("irq_enabled:%d, irq_count:%d\n", irq_enabled, irq_count);
}
#endif
...
static void bcm_rxtx_work_func(struct work_struct *work)
{
	...
	do {
		 ...
		 * 'g_rx_buffer_avail_bytes'(PZC) is calculated in
		 * bcm_ssi_tx() inside loop in work queue
		 * (bcm_rxtx_work_func) below this code.
		 * It means 'g_rx_buffer_avail_bytes' doesn't reflect
		 ...
		 * The 'g_rx_buffer_avail_bytes' can be tested inside
		...
		if (avail > g_rx_buffer_avail_bytes) {
			g_rx_buffer_avail_bytes ? ssi_tx_pzc_retries++ :
				ssi_tx_pzc_retry_delays++;
			pr_debug("%d PZC %s, wr CIRC_CNT %lu, RNGDMA_RX %lu\n",
				g_rx_buffer_avail_bytes ?
				ssi_tx_pzc_retries :
				ssi_tx_pzc_retry_delays,
				g_rx_buffer_avail_bytes ?
...
static int bcm_spi_probe(struct spi_device *spi)
{
	...
	/* Check GPIO# */
#ifndef CONFIG_OF
	pr_err("Check platform_data for bcm device\n");
#else
	if (!spi->dev.of_node) {
		pr_err("Failed to find of_node\n");
		goto err_exit;
	}
#endif
	...
	pr_info("ssp-host-req=%d, ssp-mcu_req=%d, ssp-mcu-resp=%d nstandby=%d \n",
		host_req, mcu_req, mcu_resp, nstandby);
	if (host_req < 0 || mcu_req < 0 || mcu_resp < 0 || nstandby < 0) {
		pr_err("GPIO value not correct\n");
		goto err_exit;
	}
	...
	if (ret) {
		pr_err("failed to request HOST REQ, ret:%d", ret);
		goto err_exit;
	}
	spi->irq = gpio_to_irq(host_req);
	if (spi->irq < 0) {
		pr_err("irq=%d for host_req=%d not correct\n",
				spi->irq, host_req);
		goto err_exit;
	}
	...
	if (ret) {
		pr_err("failed to request MCU REQ, ret:%d", ret);
		goto err_exit;
	}
	ret = gpio_direction_output(mcu_req, 0);
	if (ret) {
		pr_err("failed set MCU REQ as input mode, ret:%d",
				ret);
		goto err_exit;
	}
	ret = gpio_request(mcu_resp, "MCU RESP");
	if (ret) {
		pr_err("failed to request MCU RESP, ret:%d", ret);
		goto err_exit;
	}
	ret = gpio_direction_input(mcu_resp);
	if (ret) {
		pr_err("failed set MCU RESP as input mode, ret:%d",
				ret);
		goto err_exit;
	}
	...
	if (ret) {
		pr_err("failed to request GPS NSTANDBY, ret:%d", ret);
		goto err_exit;
	}
	ret = gpio_direction_output(nstandby, 0);
	if (ret) {
		pr_err("failed set GPS NSTANDBY as out mode, ret:%d",
				ret);
		goto err_exit;
	}
	...
	if (gps_power >= 0) {
		...
		if (ret) {
			pr_err("failed to request GPS POWER, ret:%d", ret);
			goto err_exit;
		}
		ret = gpio_direction_output(gps_power, 1);
		if (ret) {
			pr_err("failed set GPS POWER as out mode, ret:%d", ret);
			goto err_exit;
		}
	}
	...
	priv->spi = spi;
	priv->tx_buf = devm_kmalloc(&spi->dev, sizeof(struct bcm_ssi_tx_frame), GFP_KERNEL);
	priv->rx_buf = devm_kmalloc(&spi->dev, sizeof(struct bcm_ssi_rx_frame), GFP_KERNEL);
	if (!priv->tx_buf || !priv->rx_buf) {
		pr_err("Failed to allocate xfer buffer. tx_buf=%p, rx_buf=%p\n",
	...
	pr_notice("Probe OK. ssp-host-req=%d, irq=%d, priv=0x%p\n",
	...
	if (ret) {
		pr_err("Failed to register bcm_gps_spi's misc dev. err=%d\n", ret);
		goto free_wq;
	}
