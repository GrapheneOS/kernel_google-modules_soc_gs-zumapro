...
static int rt6160_disable(struct regulator_dev *rdev)
{
	...
	/* When we did not configure enable_gpio, the devm_gpiod_get_optional should
	 return NULL in rt6160_probe. Hence, it should return successfully here */
	if (!priv->enable_gpio)
		return 0;
...
static int rt6160_set_mode(struct regulator_dev *rdev, unsigned int mode)
{
	...
	return regmap_update_bits(priv->regmap, RT6160_REG_CNTL, RT6160_FPWM_MASK, mode_val);
}
...
static unsigned int rt6160_get_mode(struct regulator_dev *rdev)
{
	...
	ret = regmap_read(priv->regmap, RT6160_REG_CNTL, &val);
	if (ret)
		return ret;
...
static int rt6160_set_suspend_voltage(struct regulator_dev *rdev, int uV)
{
	...
	return regmap_update_bits(priv->regmap, suspend_vsel_reg,
...
static int rt6160_get_error_flags(struct regulator_dev *rdev, unsigned int *flags)
{
	...
	ret = regmap_read(priv->regmap, RT6160_REG_STATUS, &val);
	if (ret)
		return ret;
