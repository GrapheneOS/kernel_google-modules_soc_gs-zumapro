	...
	struct max77826_platform_data *pdata)
{
	...
	max77826->rdev =
		devm_kcalloc(max77826->dev, MAX77826_REG_MAX,
			    ...
			    const struct i2c_device_id *id)
{
	...
	max77826 = devm_kzalloc(&i2c->dev, sizeof(struct max77826_dev),
				GFP_KERNEL);
	if (!max77826)
		return -ENOMEM;
	...
	if (of_property_count_strings(i2c->dev.of_node, "pinctrl-names") == 0) {
		dev_err(&i2c->dev, "no pinctrl defined\n");
		return -ENODEV;
	}

	max77826->pinctrl = devm_pinctrl_get(max77826->dev);
	if (IS_ERR(max77826->pinctrl)) {
		dev_err(&i2c->dev, "Cannot allocate pinctrl\n");
		return PTR_ERR(max77826->pinctrl);
	}

	state = pinctrl_lookup_state(max77826->pinctrl, "active");
	if (IS_ERR(state)) {
		dev_err(&i2c->dev, "Cannot find pinctrl state: active\n");
		devm_pinctrl_put(max77826->pinctrl);
		return PTR_ERR(state);
	}

	ret = pinctrl_select_state(max77826->pinctrl, state);
	if (ret) {
		dev_err(&i2c->dev, "Cannot select state: active\n");
		return ret;
	}

	max77826->enable_gpio = of_get_named_gpio(max77826->dev->of_node,
						  "maxim,enable-gpio", 0);

	ret = devm_gpio_request_one(max77826->dev, max77826->enable_gpio,
...
module_i2c_driver(max77826_i2c_driver)
