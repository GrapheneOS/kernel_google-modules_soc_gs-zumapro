 ...
 *
 * USB Type-C Port Controller Core Interface Shim Layer.
 */

#ifndef __GOOGLE_TCPCI_SHIM_H
#define __GOOGLE_TCPCI_SHIM_H
...
#define tcpc_presenting_rd(REGVAL, CCNUM) \
	(!(TCPC_ROLE_CTRL_DRP & (REGVAL)) && \
	 (((REGVAL) & (TCPC_ROLE_CTRL_## CCNUM ##_MASK << TCPC_ROLE_CTRL_## CCNUM ##_SHIFT)) == \
	  (TCPC_ROLE_CTRL_CC_RD << TCPC_ROLE_CTRL_## CCNUM ##_SHIFT)))

struct google_shim_tcpci {
	struct device *dev;
 ...
 *		toggling state.
 * @get_vbus:
 *		Optional; Callback to check if vbus is present. If callback is
 *		absent this module will fetch the value by performing a regmap
 *		read else use the fetched value. If the value returned is 0,
...
struct google_shim_tcpci_data {
	...
	int (*init)(struct google_shim_tcpci *tcpci, struct google_shim_tcpci_data *data);
	int (*set_vconn)(struct google_shim_tcpci *tcpci, struct google_shim_tcpci_data *data,
			 bool enable);
	int (*start_drp_toggling)(struct google_shim_tcpci *tcpci,
				  struct google_shim_tcpci_data *data, enum typec_cc_status cc);
	int (*set_vbus)(struct google_shim_tcpci *tcpci, struct google_shim_tcpci_data *data,
			bool source, bool sink);
	void (*frs_sourcing_vbus)(struct google_shim_tcpci *tcpci,
				  struct google_shim_tcpci_data *data);
	void (*set_partner_usb_comm_capable)(struct google_shim_tcpci *tcpci,
